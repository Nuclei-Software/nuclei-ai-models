...
def evaluate_tflite_model(model_path, data_path):
    ...
    size = input_details['shape'][1]  # 296
    ...
    while data_pos < data_end:
        ...
        interpreter.set_tensor(input_details["index"], tf.cast(data[:, start:end, :], input_dtype))
        interpreter.invoke()
        cur_output_data = interpreter.get_tensor(output_details["index"])[:, :, y_start:y_end, :]
        cur_output_data = dequantize(cur_output_data, output_scale, output_zero_point)
    ...
    print(f'Transcribed File: {"".join(decoded_output)}')
