...
class YOLOxTFLite:
    ...
    def __init__(self, model: str, conf: float = 0.25, iou: float = 0.25, metadata: Union[str, None] = None):
        """
        Initialize an instance of the YOLOxTFLite class.
    ...
    def postprocess(self, img: np.ndarray, outputs: np.ndarray, ratio: Tuple[float, float]) -> np.ndarray:
        ...
        anchors = np.zeros([1, 1, 2], dtype=np.float32)
        anchors[0,0,:] = [0.5, 0.5]

        yv, xv = np.meshgrid(np.arange(ny), np.arange(nx))

        grid = np.stack((yv, xv), 2).reshape((ny, nx, 2)).astype(np.float32)
        ...
        outputs[..., 2:4] = np.exp(outputs[..., 2:4]) * anchors * INPUT_SIZE
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="../pretrained_models/coco_2017_person/yolo_x_nano_256/yolo_x_nano_256_0.5_0.4_int8.tflite",
    ...
    detector = YOLOxTFLite(args.model, args.conf, args.iou, args.metadata)
