...
# reference： https://github.com/motokimura/yolox-ti-lite_tflite/blob/main/demo/TFLite/tflite_inference.py
...
class SSDMobilenetv2TFlite:
    ...
    def __init__(self, model: str, conf: float = 0.25, iou: float = 0.25, metadata: Union[str, None] = None):
        """
        Initialize an instance of the SSDMobilenetv2TFlite class.
        ...
        # Get output details
        scores_details = self.model.get_output_details()[0]
        self.scores_index = scores_details["index"]
        self.scores_out_scale, self.scores_out_zero_point = scores_details["quantization"]

        raw_boxes_details = self.model.get_output_details()[1]
        self.raw_boxes_index = raw_boxes_details["index"]
        self.raw_boxes_out_scale, self.raw_boxes_out_zero_point = raw_boxes_details["quantization"]

        anchor_boxes_details = self.model.get_output_details()[2]
        self.anchor_boxes_index = anchor_boxes_details["index"]
        self.anchor_boxes_out_scale, self.anchor_boxes_out_zero_point = anchor_boxes_details["quantization"]
    ...
    def postprocess(self, img: np.ndarray, scores: np.ndarray, raw_boxes: np.ndarray, anchor_boxes: np.ndarray, ratio: Tuple[float, float]) -> np.ndarray:
        """
        ...
        boxes = tf.stack([x1, y1, x2 - x1, y2 - y1], axis=-1)[0].numpy() #(x, y, w, h)
        score = scores[0, ...,  1].flatten()
        class_ids = scores[0, ...,  1:].argmax(-1)
        # Apply non-maximum suppression
        indices = cv2.dnn.NMSBoxes(boxes, score, self.conf, self.iou)

        assert self.in_height == self.in_width
        BLOCK_SIZE = self.in_width

        boxes[..., [0, 2]] *= ratio[1] * BLOCK_SIZE
        boxes[..., [1, 3]] *= ratio[0] * BLOCK_SIZE

        # Draw detections that survived NMS
        [self.draw_detections(img, boxes[i], score[i], class_ids[i]) for i in indices]
    ...
    def detect(self, img_path: str) -> np.ndarray:
        ...
        scores = self.model.get_tensor(self.scores_index)
        if self.int8:
            scores = (scores.astype(np.float32) - self.scores_out_zero_point) * self.scores_out_scale
        raw_boxes = self.model.get_tensor(self.raw_boxes_index)

        if self.int8:
            raw_boxes = (raw_boxes.astype(np.float32) - self.raw_boxes_out_zero_point) * self.raw_boxes_out_scale
        anchor_boxes = self.model.get_tensor(self.anchor_boxes_index)

        if self.int8:
            anchor_boxes = (anchor_boxes.astype(np.float32) - self.anchor_boxes_out_zero_point) * self.anchor_boxes_out_scale

        # Process detections and return result
        return self.postprocess(img, scores, raw_boxes, anchor_boxes, ratio)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="../pretrained_models/coco_2017_person/ssd_mobilenet_v2_fpnlite_035_224/ssd_mobilenet_v2_fpnlite_035_224_int8.tflite",
    ...
    detector = SSDMobilenetv2TFlite(args.model, args.conf, args.iou, args.metadata)
