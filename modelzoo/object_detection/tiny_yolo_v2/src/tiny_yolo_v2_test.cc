...
#include "tiny_yolo_v2_224_int8_tflite.h"
...
void setup() {
  ...
  tinyYoloV2::model = tflite::GetModel(___pretrained_models_coco_2017_person_tiny_yolo_v2_224_int8_tflite);
...
float anchors[5][2] = {{0.076023, 0.258508}, 
                       {0.163031, 0.413531}, 
                       {0.234769, 0.702585},
                       {0.427054, 0.715892}, 
                       {0.748154, 0.857092}};
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < 7 * 7; i++) {
    for (int j = 0; j < 5; j++) {
      ...
      grid_x = i % 7;
      grid_y = i / 7;
      x = (sigmod(x) + grid_x) * (224 / 7);
      y = (sigmod(y) + grid_y) * (224 / 7);
      w = expf(w) * anchors[j][0] * (224 / 7);
      h = expf(h) * anchors[j][1] * (224 / 7);
