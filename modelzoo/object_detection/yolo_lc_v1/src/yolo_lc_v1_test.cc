...
#include "yolo_lc_v1_224_int8_tflite.h"
...
void setup() {
  ...
  tinyYoloV2::model = tflite::GetModel(___pretrained_models_coco_2017_person_yolo_lc_v1_224_yolo_lc_v1_224_int8_tflite);
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < 14 * 14; i++) {
    for (int j = 0; j < 5; j++) {
      ...
      x = (sigmod(x) + grid_x) * (224 / 14);
      y = (sigmod(y) + grid_y) * (224 / 14);
      w = expf(w) * (224 / 14);
      h = expf(h) * (224 / 14);
  ...
  // 调用NMS
  nms(boxes, num_boxes, 0.25f, &out_boxes, &out_num_boxes);
