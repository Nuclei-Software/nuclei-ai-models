...
class YOLOLcV1TFLite:
    ...
    def __init__(self, model: str, conf: float = 0.25, iou: float = 0.25, metadata: Union[str, None] = None):
        """
        Initialize an instance of the YOLOLcV1TFLite class.
    ...
    def postprocess(self, img: np.ndarray, outputs: np.ndarray, ratio: Tuple[float, float]) -> np.ndarray:
        ...
        # print(outputs[..., 0].transpose([1,2,0]))
        outputs[..., 0:2] = (sigmoid(outputs[..., 0:2]) + grid) * BLOCK_SIZE
        outputs[..., 2:4] = np.exp(outputs[..., 2:4]) * BLOCK_SIZE
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="../pretrained_models/coco_2017_person/yolo_lc_v1_224/yolo_lc_v1_224_int8.tflite",
    ...
    detector = YOLOLcV1TFLite(args.model, args.conf, args.iou, args.metadata)
