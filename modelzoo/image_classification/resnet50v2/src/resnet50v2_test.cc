...
namespace resnet50 {
  ...
  constexpr int kTensorArenaSize = 25643576;
...
void setup() {
  ...
  resnet50::model = tflite::GetModel(___pretrained_models_ImageNet_resnet50_v2_224_resnet50_v2_224_int8_tflite);
  if (resnet50::model->version() != TFLITE_SCHEMA_VERSION) {
    MicroPrintf(
        "Model provided is schema version %d not equal "
        "to supported version %d.\n",
        resnet50::model->version(), TFLITE_SCHEMA_VERSION);
  }
  ...
  static tflite::MicroInterpreter static_interpreter(
      resnet50::model, op_resolver, resnet50::tensor_arena, resnet50::kTensorArenaSize);
  resnet50::interpreter = &static_interpreter;

  // Allocate memory from the tensor_arena for the model's tensors.
  TfLiteStatus allocate_status = resnet50::interpreter->AllocateTensors();
  ...
  resnet50::input = resnet50::interpreter->input(0);

  // Make sure the input has the properties we expect
  if (resnet50::input == nullptr) {
...
int main(int argc, char* argv[]) {
  ...
  if (kTfLiteOk != GetImage(224, 224, 3, resnet50::input->data.uint8)) {
    MicroPrintf("Image capture failed.");
  }

  // Run the model on this input and make sure it succeeds.
  if (kTfLiteOk != resnet50::interpreter->Invoke()) {
    MicroPrintf("Invoke failed.");
  }

  TfLiteTensor* output = resnet50::interpreter->output(0);
