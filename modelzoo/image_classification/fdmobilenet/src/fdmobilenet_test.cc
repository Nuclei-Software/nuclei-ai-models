...
namespace fdmobilenet {
  ...
  constexpr int kTensorArenaSize = 131436;
...
void setup() {
  ...
  fdmobilenet::model = tflite::GetModel(___pretrained_models_flowers_fdmobilenet_0_25_128_tfs_int8_tflite);
  if (fdmobilenet::model->version() != TFLITE_SCHEMA_VERSION) {
    MicroPrintf(
        "Model provided is schema version %d not equal "
        "to supported version %d.\n",
        fdmobilenet::model->version(), TFLITE_SCHEMA_VERSION);
  }
  ...
  static tflite::MicroInterpreter static_interpreter(
      fdmobilenet::model, op_resolver, fdmobilenet::tensor_arena, fdmobilenet::kTensorArenaSize);
  fdmobilenet::interpreter = &static_interpreter;

  // Allocate memory from the tensor_arena for the model's tensors.
  TfLiteStatus allocate_status = fdmobilenet::interpreter->AllocateTensors();
  ...
  fdmobilenet::input = fdmobilenet::interpreter->input(0);

  // Make sure the input has the properties we expect
  if (fdmobilenet::input == nullptr) {
...
int main(int argc, char* argv[]) {
  ...
  if (kTfLiteOk != GetImage(128, 128, 3, fdmobilenet::input->data.uint8)) {
    MicroPrintf("Image capture failed.");
  }

  // Run the model on this input and make sure it succeeds.
  if (kTfLiteOk != fdmobilenet::interpreter->Invoke()) {
    MicroPrintf("Invoke failed.");
  }

  TfLiteTensor* output = fdmobilenet::interpreter->output(0);
