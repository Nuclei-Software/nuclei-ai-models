...
#include "mobilenet_v1_0_5_224_int8_tflite.h"
...
namespace mobilenet_v1 {
  ...
  constexpr int kTensorArenaSize = 1345604;
...
void setup() {
  ...
  mobilenet_v1::model = tflite::GetModel(___pretrained_models_ImageNet_mobilenet_v1_0_5_224_mobilenet_v1_0_5_224_int8_tflite);
  if (mobilenet_v1::model->version() != TFLITE_SCHEMA_VERSION) {
    MicroPrintf(
        "Model provided is schema version %d not equal "
        "to supported version %d.\n",
        mobilenet_v1::model->version(), TFLITE_SCHEMA_VERSION);
  }
  ...
  static tflite::MicroInterpreter static_interpreter(
      mobilenet_v1::model, op_resolver, mobilenet_v1::tensor_arena, mobilenet_v1::kTensorArenaSize);
  mobilenet_v1::interpreter = &static_interpreter;

  // Allocate memory from the tensor_arena for the model's tensors.
  TfLiteStatus allocate_status = mobilenet_v1::interpreter->AllocateTensors();
  ...
  mobilenet_v1::input = mobilenet_v1::interpreter->input(0);

  // Make sure the input has the properties we expect
  if (mobilenet_v1::input == nullptr) {
...
int main(int argc, char* argv[]) {
  ...
  if (kTfLiteOk != GetImage(224, 224, 3, mobilenet_v1::input->data.uint8)) {
    MicroPrintf("Image capture failed.");
  }

  // Run the model on this input and make sure it succeeds.
  if (kTfLiteOk != mobilenet_v1::interpreter->Invoke()) {
    MicroPrintf("Invoke failed.");
  }

  TfLiteTensor* output = mobilenet_v1::interpreter->output(0);
