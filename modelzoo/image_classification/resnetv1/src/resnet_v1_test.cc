...
#include "resnet_v1_8_32_tfs_int8_tflite.h"
...
namespace resnet_v1 {
  ...
  constexpr int kTensorArenaSize = 78768;
...
void setup() {
  ...
  resnet_v1::model = tflite::GetModel(___pretrained_models_cifar10_resnet_v1_8_32_tfs_resnet_v1_8_32_tfs_int8_tflite);
  if (resnet_v1::model->version() != TFLITE_SCHEMA_VERSION) {
    MicroPrintf(
        "Model provided is schema version %d not equal "
        "to supported version %d.\n",
        resnet_v1::model->version(), TFLITE_SCHEMA_VERSION);
  }
  ...
  static tflite::MicroInterpreter static_interpreter(
      resnet_v1::model, op_resolver, resnet_v1::tensor_arena, resnet_v1::kTensorArenaSize);
  resnet_v1::interpreter = &static_interpreter;

  // Allocate memory from the tensor_arena for the model's tensors.
  TfLiteStatus allocate_status = resnet_v1::interpreter->AllocateTensors();
  ...
  resnet_v1::input = resnet_v1::interpreter->input(0);

  // Make sure the input has the properties we expect
  if (resnet_v1::input == nullptr) {
...
int main(int argc, char* argv[]) {
  ...
  if (kTfLiteOk != GetImage(32, 32, 3, resnet_v1::input->data.uint8)) {
    MicroPrintf("Image capture failed.");
  }

  // Run the model on this input and make sure it succeeds.
  if (kTfLiteOk != resnet_v1::interpreter->Invoke()) {
    MicroPrintf("Invoke failed.");
  }

  TfLiteTensor* output = resnet_v1::interpreter->output(0);

  // Process the inference results.
  float *output_array = output->data.f;
  int output_array_len = 10;
  int k = 3; // Top-3
