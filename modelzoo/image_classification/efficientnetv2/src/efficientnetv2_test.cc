...
namespace efficientnetv2 {
  ...
  constexpr int kTensorArenaSize = 7243924;
...
void setup() {
  ...
  efficientnetv2::model = tflite::GetModel(___pretrained_models_ImageNet_efficientnet_v2B0_224_int8_tflite);
  if (efficientnetv2::model->version() != TFLITE_SCHEMA_VERSION) {
    MicroPrintf(
        "Model provided is schema version %d not equal "
        "to supported version %d.\n",
        efficientnetv2::model->version(), TFLITE_SCHEMA_VERSION);
  }

  static tflite::MicroMutableOpResolver<14>  op_resolver;
  ...
  static tflite::MicroInterpreter static_interpreter(
      efficientnetv2::model, op_resolver, efficientnetv2::tensor_arena, efficientnetv2::kTensorArenaSize);
  efficientnetv2::interpreter = &static_interpreter;

  // Allocate memory from the tensor_arena for the model's tensors.
  TfLiteStatus allocate_status = efficientnetv2::interpreter->AllocateTensors();
  ...
  efficientnetv2::input = efficientnetv2::interpreter->input(0);

  // Make sure the input has the properties we expect
  if (efficientnetv2::input == nullptr) {
...
int main(int argc, char* argv[]) {
  ...
  if (kTfLiteOk != GetImage(224, 224, 3, efficientnetv2::input->data.uint8)) {
    MicroPrintf("Image capture failed.");
  }

  // Run the model on this input and make sure it succeeds.
  if (kTfLiteOk != efficientnetv2::interpreter->Invoke()) {
    MicroPrintf("Invoke failed.");
  }

  TfLiteTensor* output = efficientnetv2::interpreter->output(0);
